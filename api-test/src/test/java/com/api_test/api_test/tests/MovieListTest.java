...
public class MovieListTest extends BaseTest {
    ...
    public void testMovieList_HasPaginationParams() {
        RestAssured
            ...
            .then()
                ...
                .body("data.params.pagination.totalPages", greaterThan(0));
    ...
    @Test
    @DisplayName("❌ [FAIL] hoat-hinh slug → API bị lỗi server, test expect 'success' nhưng nhận 'error'")
    public void testMovieList_HoatHinhSlug_BugOnServer_ExpectFail() {
        // BUG THỰC TẾ: slug "hoat-hinh" trả về status:"error" + message:"Cannot read properties of null"
        // Test này INTENTIONALLY FAIL để minh họa lỗi phía server
        RestAssured
            .given()
                .spec(requestSpec)
                .queryParam("page", 1)
    ...
    @Test
    @DisplayName("❌ [FAIL] totalPages → field không tồn tại trong response, expect > 0 nhưng nhận null")
    public void testMovieList_TotalPages_FieldNotExist_ExpectFail() {
        ...
        RestAssured
            ...
            .then()
                .statusCode(200)
                .body("data.params.pagination.totalPages", greaterThan(0)); // ← FAIL: field null
    }
}
